...
namespace MozJpegFileType.Exif
{
    internal sealed class ExifWriter
    {
        ...
        private const int MaxJpegApp1PayloadLength = 65535 - sizeof(ushort);

        private readonly Dictionary<MetadataSection, Dictionary<ushort, MetadataEntry>> metadata;

        private static readonly byte[] ExifMarkerAscii = Encoding.ASCII.GetBytes("Exif\0\0");
        ...
        public byte[] CreateExifApp1Payload()
        {
            IFDInfo ifdInfo = BuildIFDEntries();
            Dictionary<MetadataSection, IFDEntryInfo> ifdEntries = ifdInfo.IFDEntries;

            long exifApp1PayloadLength = ExifMarkerAscii.LongLength + ifdInfo.EXIFDataLength;

            if (exifApp1PayloadLength > MaxJpegApp1PayloadLength)
            {
                return null;
            }

            byte[] exifBytes = new byte[checked((int)exifApp1PayloadLength)];

            using (MemoryStream stream = new MemoryStream(exifBytes))
            using (BinaryWriter writer = new BinaryWriter(stream))
            {
                writer.Write(ExifMarkerAscii);
